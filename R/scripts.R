...
#' @param output_folder Output folder. A number of files will be created in the form 
#' ‘output_folder/<prefix>_*.npy’
...
#' @param zero_perturb Flag to indicated you want to pertub the zero entries. 
#' Default: FALSE
...
PreprocessMatrix <- function(input_file, output_folder=NULL, prefix=NULL, max_bound=10, 
  ...
  reticulate::import_from_path("NumSwitch", system.file("python", package = "PressPurtCoreAlg"), convert = F)
  reticulate::source_python(system.file("python", "PreprocessMatrix.py", package = "PressPurtCoreAlg"), convert = F)
  tt <- run_preproc(input_file, output_folder, prefix, max_bound, zero_perturb, threads, verbose)
  return(py_to_r(tt))
}
...
#' This is also where the num switch array will be saved.
#' @param prefix Prefix of output files, if you so choose.
...
ComputeEntryWisePerturbationExpectation <- function(input_folder, prefix=NULL, 
  ...
  reticulate::import_from_path("SS", 
                               system.file("python", package = "PressPurtCoreAlg"), 
                               convert = F)
  reticulate::source_python(system.file("python", "ComputeEntryWisePerturbationExpectation.py", 
                                        package = "PressPurtCoreAlg"), convert = F)
  run_EntryWise(input_folder, prefix, distribution_type, input_a, input_b, threads)
}
...
GenerateEntryWiseFigures <- function(input_folder, prefix=NULL, 
    ...
    for(k in 0:(m-1)){
      for(l in 0:(n-1)){
        if(paste("(", k, ", ", l, ")", sep = '') %in% names(dists.2)){
          interval <- intervals[(k+1), (l+1),] # change back to R 1 based
          padding <- (interval[2] - interval[1])/100
          x_range <- np$linspace((interval[1] - padding), (interval[2] + padding), 250)
          dist.py <- dists.2[paste("(", k, ", ", l, ")", sep = '')]
          dist_vals <- vector()
          for(eps in x_range){
            dist_vals <- c(dist_vals, dist.py[[1]]$pdf(eps))
          }
